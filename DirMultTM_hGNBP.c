...
{
    double *DSZS, *WSZS,  *ZS, *TS, *ell_dot_k, *cjpj, gamma0, *r_k, *r_star, eta;
    ...
    double * n_vtk, *n_vk, *n_vt, *prob_cumsum_t, n_v;
    void *newptr;
            ...
            for (current_row_index =  starting_row_index; current_row_index<stopping_row_index; current_row_index++) {
                ...
                for (cum_sum=0;token< Tsize; token++) {
                    ...
                    for (cum_sum =0, k=0; k<Ksize; k++){
                        prob[k] =  (WSZS[v+k*Vsize]+eta)/(ell_dot_k[k]+Vsize*eta)*(r_k[k]+DSZS[j+Nsize*k])/cjpj[j];
                        cum_sum +=n_vk[k] + prob[k];
                     /*   //cum_sum +=prob[k];*/
                        prob_cumsum[k] = cum_sum;
                    }
                 /*   //if ( ((double) rand() / RAND_MAX * (cum_sum +1.0/Vsize * gamma0/cjpj)) < cum_sum){*/
                    if ( ((double) rand() / RAND_MAX * ( cum_sum +1.0/Vsize*r_star[0]/cjpj[j])) < cum_sum){
                        /*K will not increase*/
